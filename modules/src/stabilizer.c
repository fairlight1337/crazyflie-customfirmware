...
LOG_GROUP_START(alti)
LOG_ADD(LOG_FLOAT, pressure, &fPressure)
LOG_ADD(LOG_FLOAT, temperature, &fTemperature)
LOG_GROUP_STOP(alti)
...
static void stabilizerTask(void* param)
{
  ...
  while(1)
  {
    ...
    // NOTE(winkler): Added measurement of the magnetometer to the `mag' variable here.
    mag.x = hmc5883lGetHeadingX();
    mag.y = hmc5883lGetHeadingY();
    mag.z = hmc5883lGetHeadingZ();
    
    fPressure = ms5611GetPressure(MS5611_OSR_256);
    fTemperature = ms5611GetTemperature(MS5611_OSR_256);
  }
}
